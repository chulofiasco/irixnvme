...
nvme_shutdown(nvme_soft_t *soft)
{
...
#ifdef NVME_DBG
    cmn_err(CE_NOTE, "nvme: shutting down controller (initialized=%d)", soft->initialized);
#endif
    ...
    /*
     * Only attempt hardware shutdown if BAR0 is mapped
     * This prevents accessing invalid BAR0 mappings during error cleanup
     */
    if (soft->bar0) {
        /* Only do controller shutdown if it was initialized */
        if (soft->initialized) {
            /*
